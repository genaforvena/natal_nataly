    ...
    try:
        ...
        for i, (role, content) in enumerate(conversations, 1):
            ...
            if len(thread) == 10 and i > 10:
                print("  ⚠️  TRIMMING: Removed oldest non-fixed message (FIFO)")

            if summary['fixed_messages'] > 0 and i <= 2:
                print("  🔒 FIXED: This message will never be deleted")
        ...
        print("  3. user      | А что насчет карьеры?")
        print("  4. assistant | С Солнцем в Тельце тебе подходят профессии...")
        print("  (These were deleted to maintain the 10-message limit)")
