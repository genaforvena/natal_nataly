...
4. **ask_transit_question** - User is asking about transits, current/future planetary influences, or what's happening now/in the future
5. **new_profile_request** - User wants to create a new profile (for partner, friend, etc.)
6. **switch_profile** - User wants to switch between existing profiles
7. **ask_general_question** - User is asking general astrology questions not tied to their chart
8. **meta_conversation** - User is having casual conversation, greetings, or talking about the bot itself
9. **unknown** - Intent cannot be determined
...
**Output:** {"intent": "ask_transit_question", "confidence": 0.96}

**Input:** "What's happening in march 2026?"
**Output:** {"intent": "ask_transit_question", "confidence": 0.94}

**Input:** "Что сейчас делает Сатурн в моей карте?"
**Output:** {"intent": "ask_transit_question", "confidence": 0.93}
